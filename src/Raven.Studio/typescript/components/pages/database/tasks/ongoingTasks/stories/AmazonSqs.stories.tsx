...
import OngoingTaskAmazonSqsEtlListView = Raven.Client.Documents.Operations.OngoingTasks.OngoingTaskQueueEtl;
import { MockedValue } from "test/mocks/services/AutoMockService";
import OngoingTasksResult = Raven.Server.Web.System.OngoingTasksResult;

export default {
    title: "Pages/Database/Tasks/Ongoing tasks/Amazon SQS",
    decorators: [withStorybookContexts, withBootstrap5, withForceRerender],
} satisfies Meta;

interface AmazonSqsProps {
    disabled: boolean;
    completed: boolean;
    customizeTask: (x: OngoingTaskAmazonSqsEtlListView) => void;
    emptyScript: boolean;
...
export const Default: StoryObj<AmazonSqsProps> = {
    render: (args: AmazonSqsProps) => {
        commonInit(args.databaseType);

        const { tasksService } = mockServices;

        const mockedValue: MockedValue<OngoingTasksResult> = (x) => {
            const ongoingTask = TasksStubs.getAmazonSqsEtl();
            if (args.disabled) {
    ...
    play: async ({ canvas }) => {
        const container = within(await canvas.findByTestId("amazon-sqs-etls"));
        await userEvent.click(await container.findByTitle(/Click for details/));
    },
};

export const NotSharded: StoryObj<AmazonSqsProps> = {
    ...Default,
    args: {
        ...Default.args,
...
export const Disabled: StoryObj<AmazonSqsProps> = {
    ...Default,
    args: {
        ...Default.args,
        disabled: true,
...
export const LoadError: StoryObj<AmazonSqsProps> = {
    ...Default,
    args: {
        ...Default.args,
        loadError: true,
...
export const RuntimeError: StoryObj<AmazonSqsProps> = {
    ...Default,
    args: {
        ...Default.args,
        runtimeError: true,
...
export const EmptyScript: StoryObj<AmazonSqsProps> = {
    ...Default,
    args: {
        ...Default.args,
        completed: true,
