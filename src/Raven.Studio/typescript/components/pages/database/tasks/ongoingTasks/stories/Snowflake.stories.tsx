...
export default {
    title: "Pages/Database/Tasks/Ongoing tasks/Snowflake",
...
export const Default: StoryObj<SnowflakeProps> = {
    render: (args: SnowflakeProps) => {
        commonInit(args.databaseType);

        const { tasksService } = mockServices;

        const mockedValue: MockedValue<OngoingTasksResult> = (x) => {
            const ongoingTask = TasksStubs.getSnowflake();
            if (args.disabled) {
    ...
    play: async ({ canvas }) => {
        const container = within(await canvas.findByTestId("snowflake-etls"));
        await userEvent.click(await container.findByTitle(/Click for details/));
    },
};

export const NotSharded: StoryObj<SnowflakeProps> = {
    ...Default,
    args: {
        ...Default.args,
...
export const Disabled: StoryObj<SnowflakeProps> = {
    ...Default,
    args: {
        ...Default.args,
        disabled: true,
...
export const LoadError: StoryObj<SnowflakeProps> = {
    ...Default,
    args: {
        ...Default.args,
        loadError: true,
...
export const RuntimeError: StoryObj<SnowflakeProps> = {
    ...Default,
    args: {
        ...Default.args,
        runtimeError: true,
...
export const EmptyScript: StoryObj<SnowflakeProps> = {
    ...Default,
    args: {
        ...Default.args,
        completed: true,
