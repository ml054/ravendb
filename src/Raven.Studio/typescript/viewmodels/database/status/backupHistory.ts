...
class backupHistory extends viewModelBase {
    ...
    compositionComplete() {
        ...
        grid.init(() => this.fetchHistory(), () =>
            [
                new actionColumn<BackupHistoryItem>(grid, x => this.showDetailsFor(x), "Show", `<i class="icon-preview"></i>`, "72px", {
                ...
                new textColumn<BackupHistoryItem>(grid, //TODO: link
                    x => x.TaskName,
                    "Task Name", "18%", {
                        sortable: "string"
                    }),
    ...
    private createVirtualOperation(item: BackupHistoryItem, result: BackupResult, db: database): OperationChanged {
        return {
            ...
            Message: "Database Backup: " + db.name,
            IsPersistent: true,
            Title: "Database Export", //TODO: change to Backup History?
